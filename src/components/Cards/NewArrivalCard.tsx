...
const NewArrivalCard = ({
  ...
  return (
    <div className="group relative flex flex-col w-full bg-[#111827] rounded-2xl overflow-hidden transition-transform duration-300 hover:-translate-y-1">
      {/* ── Image ── */}
      <Link
        href={`/home-item/product/${slugDesc}-${id}`}
        className="relative w-full aspect-square overflow-hidden bg-[#1a2030]"
      >
        <Picture
          src={image}
          alt={description}
      ...
      {/* ── Footer ── */}
      <div className="bg-[#131B2E] px-4 pt-4 pb-4 flex flex-col gap-3">
        {/* Product name */}
        <Link
          href={`/home-item/product/${slugDesc}-${id}`}
          className="text-white font-bold text-sm sm:text-base leading-snug line-clamp-1 hover:text-gray-300 transition-colors"
          dangerouslySetInnerHTML={{ __html: description }}
        />
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {quantity === 0 ? (
            <button
              ...
              className="
                px-5 py-2 text-xs bg-[#2D3449] sm:text-sm font-bold tracking-widest uppercase
